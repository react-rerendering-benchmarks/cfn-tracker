...
export const TrackingPage = memo(function TrackingPage() {
  const {
    t
  } = useTranslation();
  const trackingActor = TrackingMachineContext.useActorRef();
  const authActor = AuthMachineContext.useActorRef();
  const authState = useSelector(authActor, ({
    value
  }) => value);
  const trackingState = useSelector(trackingActor, ({
    value
  }) => value);
  const authError = useSelector(authActor, ({
    context
  }) => context.error);
  const trackingError = useSelector(trackingActor, ({
    context
  }) => context.error);
  ...
  switch (authState) {
    case 'gameForm':
      return <TrackingGamePicker onSubmit={useCallback(game => authActor.send({
        type: 'submit',
        game
      }), [])} />;
    case 'loading':
