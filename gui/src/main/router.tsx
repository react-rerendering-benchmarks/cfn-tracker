...
const router = createHashRouter([{
  ...
  children: [{
    ...
    children: [{
      ...
      loader: ({
        params
      }) => GetSessions(params.userId ?? '')
    }, {
      element: <MatchesListPage />,
      path: '/sessions/:sessionId/matches/:page?/:limit?',
      loader: ({
        params
      }) => GetMatches(Number(params.sessionId), '', Number(params.page ?? 0), Number(params.limit ?? 0))
    }]
  }]
}]);
export const RouterProvider = memo(() => <ReactRouterProvider router={router} />);